...
void spl_board_init(void)
{
    ...
    {
        printf("spl_board_init() Error: no i2c device\n");
        return;
    }
    ...
    if (!voltage_update(MPU, PMIC_OP_REG_SEL_1_1) && !voltage_update(CORE, PMIC_OP_REG_SEL_1_1))
    {
        printf("VDD_MPU: 1.1V, VDD_CORE: 1.1V");
        /* Frequency switching for OPP 100 */
        mpu_pll_config(MPUPLL_M_600);
        printf(", MPU freq: 600 MHz\n");
    }
    else
    {
        printf("spl_board_init(): voltage_update failed.\n");
    }
}
#endif

#define GPIO_2_24       (64+24)
void set_DBG_LED_CPU(char enableLED)
{
    if (gpio_request(GPIO_2_24, "lcd_pclk") != 0)
    {
        /* printf("%s: gpio_request() of GPIO_2_24 (lcd_pclk) failed.\n", __func__); */
        return;
    }
...
void s_init(void)
{
    ...
    enable_DBG_LED_CPU_pin_mux();
    enable_SYSBOOT_pin_mux();
...
int fpga_pcbVersionRequest (char* pcbRevisionID, char* pcbTypeID, char* pcbAssemblyID, char* CPURevisionAtLeast2)
{
    volatile short * PCBRevisionRegister = (short *) 0x18000824;
    short PCBRevision = * PCBRevisionRegister;
    ...
    *pcbRevisionID       = ((PCBRevision & 0x0038) >> 3) + 1;
    ...
    // i103 revision has additional offset of 1 in revision
    if (*pcbTypeID == 4)
    {
        (*pcbRevisionID)++;
    }
...
void print_pcbRevisionInformation (char pcbRevisionID, char pcbTypeID, char pcbAssemblyID, char CPURevisionAtLeast2)
{
    printf ("PCB information: ionia ");
    switch (pcbTypeID)
    {
        case 1:
            printf ("i101");
        ...
        case 4:
            ...
            break;
        default:
            printf ("unknown: PCB ID: 0x%x", pcbTypeID);
            break;
    }
    printf (", Rev: %i, Assembly ID: 0x%03x, CPU: %s\r\n", pcbRevisionID, pcbAssemblyID, CPURevisionAtLeast2?"at least 2.0":"1");
}
...
{ // detect MII / RMII Ethernet PHY
    ...
    {
    #ifndef CONFIG_SPL_BUILD
        printf("%s: gpio_request() of GPIO_2_12 (lcd_data6) failed.\n", __func__);
    #endif
        return -1;
    }
    ...
    {
    #ifndef CONFIG_SPL_BUILD
        printf("%s: gpio_request() of GPIO_2_13 (lcd_data7) failed.\n", __func__);
    #endif
        return -1;
    }
    ...
    {
    #ifndef CONFIG_SPL_BUILD
        printf("%s: gpio_direction_input() of GPIO_2_12 (lcd_data6) failed.\n", __func__);
    #endif
        return -1;
    }
    ...
    {
    #ifndef CONFIG_SPL_BUILD
        printf("%s: gpio_direction_input() of GPIO_2_13 (lcd_data7) failed.\n", __func__);
    #endif
        return -1;
    }
    ...
    {
    #ifndef CONFIG_SPL_BUILD
        printf("%s: gpio_direction_input() of GPIO_2_12 (lcd_data7) failed.\n", __func__);
    #endif
        return -1;
    }
    ...
    /* Workaround. On older PCBs (before i103 Rev 3.0 including, i101 Rev 1.2) 22k resistors are assembled for RMII and MII selector SYSBOOT6 and SYSBOOT7. FPGA is also connected
     * to SYSBOOT6 and SYSBOOT7. If FPGA is not configured the FPGA pullups overwrite SYSBOOT6 and SYSBOOT7. Therefore detection fails. */
    printf ("%s: unrealistic PHY settings read. Setting to default MII to be compatible with i103 Rev 3.0, or older (when FPGA not configured)\r\n", __func__);
    printf ("GPIO_2_12_val: %d, GPIO_2_13_val: %d\r\n", GPIO_2_12_val, GPIO_2_13_val);
    return 1;
}
...
int board_init(void)
{
    ...
    strcpy(header.name, "i10x");
    strcpy(header.version, "1.00");
    strcpy(header.serial, "1234567890");
    strcpy(header.config,  "BASE");
    header.mac_addr[0][0] = 0x00;
    header.mac_addr[0][1] = 0x01;
    header.mac_addr[0][2] = 0x02;
    header.mac_addr[0][3] = 0x03;
    header.mac_addr[0][4] = 0x04;
    header.mac_addr[0][5] = 0x05;

    board_id = DUAGON_I10X_BOARD;
    ...
    int mii_detect = detectMiiPHY();
    if (mii_detect == 1)
    {
        printf ("MII configuration detected\r\n");
        configure_i10x_mii_pin_mux(board_id, header.version, profile, daughter_board_connected);
    }
    else if (mii_detect == 0)
    {
        printf ("RMII configuration detected\r\n");
        configure_i10x_rmii_pin_mux(board_id, header.version, profile, daughter_board_connected);
    }
    else
        printf ("PHY configuration unknown\r\n");
#endif
    ...
    /*Ti 81xx NAND flash init gpmc*/
    gpmc_init();

#ifndef CONFIG_SPL_BUILD
    /*Configures NAND, FRAM and FPGA physical address mapping*/
    gpmc_init_duagon_i10x();
    tps65910_set_external_crystal();
    enable_DBG_LED_CPU_pin_mux();
    set_DBG_LED_CPU(0);
    struct rtc_time rtc_time_current = {};
    rtc_get (&rtc_time_current);
    if (rtc_time_current.tm_isdst) /* Is date set already ? (gets lost at power down if battery is empty or not assembled */
    {
        cpu_rtc_ss_set (&rtc_time_current);
    }
    else
    {
        printf ("PMIC RTC date invalid. (date never set, backup battery empty or missing).\r\n");
    }
    ...
    {
        printf ("PCB information not available. (GPMC read access from FPGA did not succeed)\r\n");
    }
#endif
    return 0;
}
...
static void evm_phy_init(char *name, int addr)
{
    ...
    unsigned int cntr = 0;
    unsigned short phyid1 = 0;
    unsigned short phyid2 = 0;
    int bone_pre_a3 = 0;

    debug("evm_phy_init(%d)\n", addr);
    ...
    if ((miiphy_read(name, addr, MII_PHYSID1, &phyid1) != 0) ||
      ...
      debug("evm_phy_init() - returned\n");
      return;
    }

    if ((phyid1 == TLK110_PHYIDR1) && (phyid2 == TLK110_PHYIDR2)) {
        debug("evm_phy_init() - detected TLK110\n");
...
int board_eth_init(bd_t *bis)
{
    ...
    {
        /* Enable RMII Mode */
        writel(RMII_MODE_ENABLE_EXT_CLK, MAC_MII_SEL);
    }
    /* No Gigabit */
