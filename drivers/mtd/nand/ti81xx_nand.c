...
#endif
static struct nand_ecclayout hw_bch8_nand_oob = GPMC_NAND_HW_BCH8_ECC_LAYOUT_112; /* Todo check if NAND type changed, it is in include/arch/arm/asm...*/
				...
				uint8_t *read_ecc, uint8_t *calc_ecc)
{
	...
	if (elm_check_error(syndrome, bch->nibbles, &error_count, error_loc) != 0) {
		 ...
		 */
		printf("ECC: uncorrectable, too many bit errors.\n");
#endif
		return -1;
	}
	...
	if (res) {
		...
		if (hm == 12) {
			...
			if ((orig_ecc == 0x0FFF0FFF) && (new_ecc == 0x00000000))
				return 0;
			printf("ti81xx_nand.c: Error: Bad compare! failed\n");
            printf ("orig_ecc: %08x\n",orig_ecc);
            printf ("new_ecc:  %08x\n",new_ecc);
				...
				uint8_t *buf, int page)
{
	...
	for (i = 0; eccsteps; eccsteps--, i += eccbytes, p += eccsize,
        ...
        /* TODO: check: patch from SPANSION suggests:
         *
		...
		nand_ecc_modes_t hardware, int32_t mode)
{
	...
	if (hardware == NAND_ECC_HW) {
		if (mode) {
			...
			nand->ecc.steps = 8;/*Todo correct this flash dependent*/
			nand->ecc.size = 512;
 ...
 * - cmd_ctrl: hardwarespecific function for accessing control-lines
 * - waitfunc: hardwarespecific function for accessing device ready/busy line
